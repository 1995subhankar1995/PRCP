...
parser.add_argument('-do', '--delta_ours', default=6, type=float, help='L2 bound on the adversarial noise')

parser.add_argument('-e', '--splits', default=10, type=int, help='Number of experiments to estimate coverage')
...
parser.add_argument('--T', default=20, type=int, help='Number of attacks')
...
for sg in range(len(act_ns_list)):
    ...
    if My_model:
        if dataset == "CIFAR10":
            if model_type == 'ResNet':
                ...
                state = torch.load(
                    '../Adversarial/checkpoints/CIFAR10_ResNet110_Robust_sigma_' + str(
                        sigma_model) + '.pth.tar',
                    map_location=device)
            elif model_type == 'DenseNet':
                model = DenseNet(depth=100, num_classes=10, growthRate=12)
                state = torch.load(
                    '../Adversarial/checkpoints/CIFAR10_DenseNet_sigma_' + str(
                        sigma_model) + '.pth.tar',
                    map_location=device)
            elif model_type == 'VGG':
            ...
            if model_type == 'ResNet':
                ...
                state = torch.load(
                    '../Adversarial/checkpoints/ResNet110_Robust_sigma_' + str(
                        sigma_model) + '.pth.tar',
                    map_location=device)
            elif model_type == 'DenseNet':
    ...
    # load cohen and salman models
        ...
        if dataset == "CIFAR10":
            if Salman:
                ...
                checkpoint = torch.load(
                    './Pretrained_Models/Cohen/models/cifar10/resnet110/noise_' + str(
                        sigma_model) + '/checkpoint.pth.tar',
                    map_location=device)
            model = get_architecture(args, checkpoint["arch"], "cifar10")
        elif dataset == "ImageNet":
            checkpoint = torch.load(
    ...
    directory = "./Adversarial_Examples_Nattacks/" + str(dataset) + "/" + str(dataset) + "/epsilon_" + str(
    ...
    mu_0 = torch.randn(x_test[0].shape)
    std_0 = torch.randn(x_test[0].shape)
    std_0 = torch.renorm(std_0, p = 2, dim = 0, maxnorm = sigma_smooth)
    ...
    if ((dataset != 'ImageNet') and (n_test != 10000)) or (
        ...
        #                                      GPU_CAPACITY=GPU_CAPACITY)
        x_test_adv = Nattack(args, mu_0, std_0, testloader, model, sigma_smooth)
        ...
        if ((dataset == 'ImageNet') and (n_test == 50000)) \
            ...
            pickle.dump([x_test_adv, x_test_adv], open(directory + "/data.pickle", 'wb'), protocol=4)
    ...
    stat_list = [
        ('ours_sc_size', si_list_ours[1]),
        ('ours_sc_cvg', cvg_list_ours[1]),
        ('RSCP_sc_size', si_list_ro[1]),
        ('RSCP_sc_cvg', cvg_list_ro[1]),

        ('ours_hcc_size', si_list_ours[0]),
        ('ours_hcc_cvg', cvg_list_ours[0]),
        ('RSCP_hcc_size', si_list_ro[0]),
        ('RSCP_hcc_cvg', cvg_list_ro[0]),

        ('vanilla_sc_size', si_list_base0[1]),
        ('vanilla_sc_cvg', cvg_list_base0[1]),
        ('cp_ss_sc_size', si_list_base1[1]),
        ('cp_ss_sc_cvg', cvg_list_base1[1]),

        ('vanilla_hcc_size', si_list_base0[0]),
        ('vanilla_hcc_cvg', cvg_list_base0[0]),
        ('cp_ss_hcc_size', si_list_base1[0]),
        ('cp_ss_hcc_cvg', cvg_list_base1[0]),
        ('top_1_clean', [acc]),
        ('top_1_adv', [adv_acc]),
        ('alphas_hcc', alphas[0]),
        ('alphas_sc', alphas[1]),
        ('clean_test_hcc_cvr', clean_test_cvr[0]),
        ('clean_test_hcc_size', clean_test_size[0]),
        ('clean_test_sc_cvr', clean_test_cvr[1]),
        ('clean_test_sc_size', clean_test_size[1]),
        ('s_APS', s_APS),
        ('s_HPS', s_HPS)
    ]
    ...
    torch.save(stat_list, patha + '/stats.pt')
