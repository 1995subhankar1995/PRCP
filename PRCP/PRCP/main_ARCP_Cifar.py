...
parser.add_argument('-do', '--delta_ours', default=0.125, type=float, help='L2 bound on the adversarial noise')
...
for sg in range(len(act_ns_list)):
    ...
    patha = '../RESULTS_UAI_26thJan/' + '/dataset_' + str(
        args.dataset) + '/network_' + str(args.arc) + '/alpha_' + str(args.alpha) + '/delta_' + str(
        args.delta) + '/num_exp_' + str(args.splits) + '/ratio_' + str(args.ratio) + '/ours_delta_' + str(
        args.delta_ours)
    patha += '/sigma_smooth_' + str(sigma_smooth) + '/sigma_model_' + str(sigma_model) + '/num_uni_noise_' + str(act_ns_list[sg])
    ...
    n_smooth = 32 #act_ns_list[sg]  # number of samples used for smoothing
    ...
    #print(f"si_list_ours = {si_list_ours}, {acc}, {alphas}, {clean_test_cvr}")
